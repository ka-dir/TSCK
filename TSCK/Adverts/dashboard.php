...
<head>
    ...
    <?php
        ...
        if(isset($_GET['xD']))
        {
			$encode_advertno=$_GET['xD'];
			$advert_no=base64_decode($encode_advertno);
			
            $sql=mysqli_query($dbconnection,"SELECT advert_no,post_vacancy FROM tblappliedjobs WHERE advert_no='".$advert_no."'")or die(mysqli_error($dbconnection));
	...
	tbl.id_number,tbl.tscNo,tbl.advert_no,tbl.post_vacancy,tbl.is_submitted,tbl.status,tbl.DateTime as submitted_time
	FROM tblappliedjobs as tbl 
	where tbl.is_submitted='".$is_submittedSubmit."'  AND tbl.advert_no='".$advert_no."' ")
	or die(mysqli_error());
...
$sqlIncomplete=mysqli_query($dbconnection,"SELECT DISTINCT 
	tbl.id_number,tbl.tscNo,tbl.advert_no,tbl.post_vacancy,tbl.is_submitted,tbl.status,tbl.DateTime as submitted_time
	FROM tblappliedjobs as tbl 
	where tbl.is_submitted='".$is_submittedInComplete."' AND tbl.advert_no='".$advert_no."' ")
	or die(mysqli_error());
$countIncomplete=mysqli_num_rows($sqlIncomplete);
	...
	tbl.id_number,CONCAT(a.title,' ',a.F_name,' ',a.S_name,' ',a.O_name) AS Name,tbl.tscNo,a.DOB AS Age,a.gender,a.KRA_pin,
	CONCAT(a.postal_address,' ',a.postal_code,' ',a.town) as address,a.county,
	tbl.advert_no,tbl.post_vacancy,tbl.status,
