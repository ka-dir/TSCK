...
if(isset($_POST['btnIinsertNew']))
{
    ...
    $filename = $_FILES["upload_cert"]["name"];
    $tempname = $_FILES["upload_cert"]["tmp_name"];
    ...
    $orig = isset($_FILES['upload_cert']) && isset($_FILES['upload_cert']['tmp_name']) ? $_FILES['upload_cert']['tmp_name'] : '';
    if (empty($orig)) {
        echo "Uploaded file doesn't exist.";
        print_r($_FILES);
    }
    $copied = move_uploaded_file($orig, $folder);
    if ($copied) {
        echo "File moved.";
    }
    else {
    ...
    /*  // Now let's move the uploaded image into the folder: image
      if( copy($_FILES['upload_cert']['tmp_name'], $folder)) {
        ...
        <form name="frmAcademic4" action="" method="post" onsubmit="return validateForm()">
            <table cellpadding="0"  cellspacing="0" border="0" class="table table-striped table-bordered" id="example" >
                ...
                <tr>
                    ...
                    <th style="text-align:center;  font-size:12px; ">Uploaded Certicate</th>
            ...
            ?>
			  ...
			  <!--<input name="specialisation" type="text" required="required" style="font-weight:bold;" value="<?php echo $row['specialisation'] ?>" />-->
                ...
                ?>
                    ...
                    <td>
                        <span class="controls">  <input name="upload_cert"  id="fileName" type="file" required="required" style=" font-weight:bold;" /></span>
                    </td>
...
<input name="cert_year" id="txtGRADDate"  onchange="TDateGraduate()" type="date" required="required" value="<?php echo $row['cert_year'] ?>" />
    ...
    <a  href="././sectionFour/uploads/test_link.php">test_link</a>
    </body>
    ...
    fileName.addEventListener('change', (event) => {
        ...
        if (target.files && target.files[0]) {
            ...
            if (target.files[0].size > maxAllowedSize) {
                ...
                alert("Please Upload Passport size less than 5MB");
                target.value = ''
            }
        }
    })
