...
// the likes of akina abdi
if($VarTscNo>0)
{
echo"this is a TSC member";
//..............condition 1 for logic 1
$sql="SELECT DISTINCT * FROM `applicant_details` WHERE `id_no`='".$VarID_No."' and tscNo='".$VarTscNo."'";
$resultA=$conn->query($sql)or die(mysqli_error($conn));
$countA=mysqli_num_rows( $resultA);

   if($countA==1)
    {
        //this is abdi revisit
	...
	tscNo='$vartscNo',
	KRA_pin='$varKRA_pin',
	mobile_no='$varmobile_no',
	gender='$vargender',
	nationality='$varnationality',
	county='$varcounty',
	sub_county='$varsub_county',
	constituency='$varconstituency',
	postal_address='$varpostal_address',
	postal_code='$varpostal_code',
	town='$vartown',
	email='$varemail',
	alt_person_name='$varalt_person_name',
	alt_tel_no='$varalt_tel_no',
	disability='$vardisability',
	disability_description='$vardisability_description',
	disability_no='$vardisability_no',
	disability_reg_date='$vardisability_reg_date',
	current_employer_name='$varcurrent_employer_name',
	position_held='$varposition_held',
	effective_date='$vareffective_date',
	gross_salary='$vargross_salary'
	...
	elseif($countA!=1)
	{
//here it means they are tsc members but new in this system
$sqlPayroll="SELECT DISTINCT * FROM staffreg WHERE Payroll_Num='".$VarTscNo."'";
$resultPayroll=$conn->query($sqlPayroll);
$countPayroll=mysqli_num_rows($resultPayroll);

	if($countPayroll==1)
	{
		echo"in the payroll but new to the services system so need to insert";
//.....lets pick what is relevant from payroll 		
				while($row = mysqli_fetch_array($resultPayroll))
					{
						$varTSCNoPayroll=intval($row['Payroll_Num']);
						$varTitlePayroll=$row['Salutation'];
						$varSurnamePayroll=$row['Surname'];
						$varFirstNamePayroll=$row['First-Name'];
						$varOtherNamesPayroll=$row['Other-Names'];
						$varBirthdatePayroll=$row['Birth_Date'];
						$varIdNumberPayroll=$row['ID_Number'];
						$varGenderPayroll=$row['Gender'];
						$varTaxPinPayroll=$row['Tax-PIN'];
						$varHomecountyPayroll=$row['County-Name'];
						$varPayrollNumPayroll=$row['Payroll_Num'];
						$varStationNamePayroll=$row['Station-Name'];
						$varDesigNamePayroll=$row['Desig-Name'];
						$varJobDesigPayroll=$row['Job-Desig'];
						$varDatePayroll=date('d-M-Y',strtotime($row['Birth_Date']));
						$varDateOfPostPayroll=date('d-M-Y',strtotime($row['Date_of_Post']));
...
$countA=mysqli_num_rows( $resultA);
   if($countA==1)
    {
		echo"this is a non tsc member but in the services system";
        //this is abdi revisit
      while($row = mysqli_fetch_array($resultA))
        {
            $varS_name=$row['S_name'];
...
elseif($countA!=1)
{
echo "<input name=\"title\" type=\"text\" readonly = \"\" value = \"$varTitlePayroll";
echo "\">";
}
else
{
...
<?php
	...
	elseif($countA==0)
	{
	echo "<input name=\"surname\" type=\"text\" readonly = \"\" value = \"$varSurnamePayroll";
	echo "\">";
	}
	else
	{
...
<?php
	...
	elseif($countA!=1)
	{
	echo "<input name=\"firstname\" type=\"text\" readonly = \"\" value = \"$varFirstNamePayroll";
	echo "\">";
	}
	else
	{
...
<?php
	...
	elseif($countA!=1)
	{
	echo "<input name=\"othername\" type=\"text\" readonly = \"\" value = \"$varOtherNamesPayroll";
	echo "\">";
	}
	else
	{
...
elseif($countA!=1)
{
echo "<input name=\"DOB\" type=\"date\" readonly = \"\" value = \"$varBirthdatePayroll";
echo "\">";
}
else
{
...
elseif($countA!=1)
{
echo "<input name=\"gender\" type=\"text\" readonly = \"\" value = \"$varGenderPayroll";
echo "\">";
}
else
{
...
<?php
	...
	echo "\">";
	  ...
	  elseif($countA!=1)
	  {
	echo "<input name=\"pinNo\" type=\"text\" readonly = \"\" value = \"$varTaxPinPayroll";
	echo "\">";
...
elseif($countA!=1)
{
	echo "<input name=\"county\" type=\"text\" readonly = \"\" value = \"$varHomecountyPayroll";
	echo "\">";
